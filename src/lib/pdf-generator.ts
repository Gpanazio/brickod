...
export function generateCallSheetPDF(callSheet: CallSheet) {
  ...
  if (callSheet.producer) {
    yPosition = addContentLine("Produtor:", callSheet.producer, yPosition);
  }
  if (callSheet.director) {
    yPosition = addContentLine("Diretor:", callSheet.director, yPosition);
  }
  if (callSheet.client) {
    yPosition = addContentLine("Cliente:", callSheet.client, yPosition);
  }
  if (callSheet.scriptName) {
    yPosition = addContentLine("Roteiro:", callSheet.scriptName, yPosition);
  }
  if (callSheet.startTime) {
    yPosition = addContentLine("Início:", callSheet.startTime, yPosition);
  }
  if (callSheet.lunchBreakTime) {
    yPosition = addContentLine("Almoço:", callSheet.lunchBreakTime, yPosition);
  }
  if (callSheet.endTime) {
    yPosition = addContentLine("Fim:", callSheet.endTime, yPosition);
  }
  ...
  if (callSheet.scenes.length > 0) {
    ...
    callSheet.scenes.forEach((scene) => {
      ...
      if (scene.estimatedTime) {
        yPosition = addContentLine("Tempo estimado:", scene.estimatedTime, yPosition);
      }
    });
    yPosition += blockSpacing;
  }
